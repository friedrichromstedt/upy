...
class TestUndarray(unittest.TestCase):
	...
	def test_multiplication_scalar(self):
		...
		self.assert_(str(en1 * (1 / en2)) == '(0.0 +- 2.0) 10^-1 ')
		...
		self.assert_(str(2 / en1) == '(1.00 +- 0.10) 10^0 ')  # unintended
...
class TestPrintableElement:
	...
	def test_sign_print(self):
		...
		p1 = upy.printable.PrintabelElement(-0.12, 0.12)
		self.assert_(str(p1) == '(-1.2 +- 1.2) 10^-1 ')

		p2 = upy.printable.PrintabelElement(-0.12, 0.12, format = 'float')
		...
		self.assert_(str(p5) == '(1.2 +- 0.012) 10^+1 ')
