...
class PrintableElement:
	...
	def __init__(self, value, uncertainty,
		...
		None, or 'float', 'int' or 'exp'.  In format 'float', no '10^...' will 
		be output, but the number of digits is in princinple infinte.  In
		format 'int', additionally all digits behind the point will be 
		suppressed.  Default is 'int' for VALUEs with an i- or u-kind dtype, 
		for all others it's 'exp'.  'exp' mode interprets all VALUEs given as 
		floats.  PRECISION influences the precision of the output.  Generally, 
		the precision is determined from the uncertainty.  With PRECISION = 1, 
		...
		in float or exp mode.  In 'int' mode, everything is limited to digits 
		before the point.  If additionally to the UNCERTAINTY also the VALUE 
		is zero, only (0 +- 0) will be printed.  The default PRECISION is 2."""
